...
module_ids = [1, 2, 3]
image_files = ["module1.png", "module2.png", "module3.png"]
image_folder = r"C:\temp"

for module_id, filename in zip(module_ids, image_files):
    file_path = os.path.join(image_folder, filename)
    if not os.path.exists(file_path):
        print(f"❌ Dosya bulunamadı: {file_path}")
        continue
    ...
    print(f"✅ Modül {module_id} için kapak resmi ayarlandı: {filename}")

conn.commit()
cursor.close()
conn.close()
print("\n🎉 Tüm modül kapak görselleri başarıyla eklendi.")
